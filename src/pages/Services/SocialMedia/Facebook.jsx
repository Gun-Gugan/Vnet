...
const Card = ({ icon, title, desc }) => (
  <div className="flex items-start gap-4 mb-6 p-4 rounded-xl shadow hover:shadow-2xl hover:scale-105 transition-transform duration-300 bg-white w-full">
...
const TestimonialCard = ({ img, title, desc }) => (
  <div className="bg-white p-6 rounded-2xl shadow-lg relative hover:shadow-2xl transition duration-300 w-full">
...
export default function FacebookMarketing() {
  return (
    <div className="space-y-32 bg-gray-50">
        ...
        <div className="container mx-auto flex flex-col-reverse lg:flex-row items-center gap-12">
          <div className="lg:w-7/12 w-full text-center lg:text-left">
            <h1 className="text-4xl lg:text-5xl font-bold mb-4">
              Maximize Your Business Growth with Strategic Facebook Marketing
            </h1>
          ...
          <div className="lg:w-5/12 w-full text-center">
            <img src={HeroImg} alt="Facebook Marketing" className="mx-auto w-3/4 lg:w-4/5 rounded-full shadow-2xl border-4 border-white" />
          </div>
        </div>
      </header>

      {/* Product Cards */}
      <section id="product" className="container mx-auto space-y-12 px-4 sm:px-6 lg:px-0">
        ...
        <div className="flex flex-col lg:flex-row items-start gap-8">
          <div className="lg:w-4/12 w-full">{cardsLeft.map((card, idx) => <Card key={idx} {...card} />)}</div>
          <div className="lg:w-4/12 w-full text-center">
            <img src={FeatureImg} alt="Facebook Features" className="w-3/4 sm:w-2/3 mx-auto rounded-full shadow-2xl" />
          </div>
          <div className="lg:w-4/12 w-full">{cardsRight.map((card, idx) => <Card key={idx} {...card} />)}</div>
        </div>
      </section>

      {/* Features Section */}
      <section id="features" className="container mx-auto space-y-12 px-4 sm:px-6 lg:px-0">
        <div className="text-center max-w-2xl mx-auto mb-12">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12">
          ...
          <div className="lg:w-7/12 w-full grid grid-cols-1 sm:grid-cols-2 gap-8">
      ...
      {/* Testimonials */}
      <section id="testimonials" className="container mx-auto space-y-12 px-4 sm:px-6 lg:px-0">
